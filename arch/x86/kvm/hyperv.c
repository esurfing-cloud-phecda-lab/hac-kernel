			   ...
			   struct pvclock_vcpu_time_info *hv_clock)
{
	...
	u64 gfn;
	int dsm_release = 0;
	// dsm_debug_v("enter kvm_hv_setup_tsc_page\n");
	...
	dsm_debug_v("kvm_hv_setup_tsc_page before kvm_dsm_acquire gfn=%llu\n", gfn);
	if (kvm_dsm_acquire(kvm, &slots, gfn_to_gpa(gfn),
				sizeof(hv->tsc_ref), true) < 0){
		dsm_debug_v("kvm_hv_setup_tsc_page after kvm_dsm_acquire gfn=%llu goto out_unlock\n", gfn);
		goto out_unlock;
	}
		dsm_debug_v("kvm_hv_setup_tsc_page after kvm_dsm_acquire gfn=%llu\n", gfn);
...
out_unlock:
	if (dsm_release){
		dsm_debug_v("kvm_hv_setup_tsc_page before kvm_dsm_release gfn=%llu\n", gfn);
		kvm_dsm_release(kvm, slots, gfn_to_gpa(gfn), sizeof(hv->tsc_ref));
	}
	mutex_unlock(&hv->hv_lock);
	// dsm_debug_v("exit kvm_hv_setup_tsc_page\n");
