...
void kvm_lapic_sync_from_vapic(struct kvm_vcpu *vcpu)
{
	...
	printk(KERN_ERR "kvm_lapic_sync_from_vapic: before kvm_dsm_vcpu_acquire(vcpu=%p, slots=%p, gpa=%llu) <0\n", vcpu, slots, vcpu->arch.apic->vapic_cache.gpa);
	if (kvm_dsm_vcpu_acquire(vcpu, &slots, vcpu->arch.apic->vapic_cache.gpa,
				  sizeof(u32), false) < 0)
		{
		printk(KERN_ERR "kvm_lapic_sync_from_vapic: kvm_dsm_vcpu_acquire(vcpu=%p, slots=%p, gpa=%llu) <0\n", vcpu, slots, vcpu->arch.apic->vapic_cache.gpa);
...
out:
	printk(KERN_ERR "kvm_lapic_sync_from_vapic: before kvm_dsm_vcpu_release(vcpu=%p, slots=%p, gpa=%llu) <0\n", vcpu, slots, vcpu->arch.apic->vapic_cache.gpa);
	kvm_dsm_vcpu_release(vcpu, slots, vcpu->arch.apic->vapic_cache.gpa,
				  sizeof(u32));
	printk(KERN_ERR "kvm_lapic_sync_from_vapic: after kvm_dsm_vcpu_release(vcpu=%p, slots=%p, gpa=%llu) <0\n", vcpu, slots, vcpu->arch.apic->vapic_cache.gpa);
}
