 ...
 */
static int __pte_list_add(struct kvm_mmu_memory_cache *cache, u64 *spte,
			struct kvm_rmap_head *rmap_head,
			void *(*allocator)(struct kvm_mmu_memory_cache *cache),
			bool check)
{
	...
	if (!rmap_head->val) {
		if (check)
			printk("pte_list_add: %p %llx 0->1\n", spte, *spte);
		rmap_head->val = (unsigned long)spte;
	} else if (!(rmap_head->val & 1)) {
		if (check)
			printk("pte_list_add: %p %llx 1->many\n", spte, *spte);
		desc = allocator(cache);
		if (!desc)
			return -ENOMEM;
		...
		if (check)
			printk("pte_list_add: %p %llx many->many\n", spte, *spte);
...
static inline int pte_list_add(struct kvm_mmu_memory_cache *cache, u64 *spte,
			struct kvm_rmap_head *rmap_head)
{
	return __pte_list_add(cache, spte, rmap_head, kvm_mmu_memory_cache_alloc, false);
}
...
static int direct_map(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	ret = mmu_set_spte(vcpu, fault->slot, it.sptep, fault->pte_access,
...
{
	dsm_debug_v("sptep=%p, old_spte=%llx, new_spte=%llx\n", sptep, old_spte, new_spte);
	/*
...
static int fast_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	do {
		dsm_debug_v("fast_page_fault do\n");
		u64 new_spte;
...
bool kvm_mmu_is_mmio_gfn(struct kvm *kvm, struct kvm_memory_slot *memslot,
		gfn_t gfn)
{
	struct kvm_rmap_head *rmap_head = gfn_to_rmap(gfn, PG_LEVEL_4K,
			memslot);
	u64 *sptep;
	struct rmap_iterator iter;
...
static int direct_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	...
	struct kvm_memory_slot *slot;
	int dsm_access;
	int write = fault->error_code & PFERR_WRITE_MASK;
...
#ifdef CONFIG_KVM_DSM
	dsm_debug_v("kvm[%d]-cpu[%d] #pf[%llx,%x](%s) gfn[%llu,%d] fault->gfn=%lld fault->pfn=%lld\n",
			vcpu->kvm->arch.dsm_id, vcpu->vcpu_id, fault->addr, fault->error_code,
			write ? "W" : (fault->error_code & PFERR_FETCH_MASK ? "RI" : "R"),
			fault->gfn, is_smm(vcpu), fault->gfn, fault->pfn);
	// 获取页表dsm权限
	dsm_access = kvm_dsm_vcpu_acquire_page(vcpu, &slot, fault->gfn, write);
	dsm_debug_v("dsm_access: %d, write: %d\n",dsm_access, write);
	if (dsm_access < 0) {
		kvm_release_pfn_clean(fault->pfn);
		return dsm_access;
	}

	fault->pte_access = dsm_access;
#else
	fault->pte_access = ACC_ALL;
#endif
...
out_unlock:
	write_unlock(&vcpu->kvm->mmu_lock);
	kvm_dsm_vcpu_release_page(vcpu, slot, fault->gfn);
	kvm_release_pfn_clean(fault->pfn);
	return r;
}
...
#ifdef CONFIG_KVM_DSM
static void *dsm_alloc_pte_list_desc(struct kvm_mmu_memory_cache *cache)
{
	...
	for_each_rmap_spte_nocheck(&slot->rmap[vfn - slot->base_vfn], &iter, entry) {
		...
		switch (dsm_access) {
			...
			case DSM_MODIFIED: /* should build spte in set_spte */
				...
				dsm_debug_v("before call gfn_to_rmap(gfn=%llx, %d, memslot=%p)\n", gfn, PG_LEVEL_4K, memslot);
				rmap_head = gfn_to_rmap(gfn, PG_LEVEL_4K, memslot);
				dsm_debug_v("after call gfn_to_rmap(gfn=%llx, %d, memslot=%p) rmap_head=%p\n", gfn, PG_LEVEL_4K, memslot, rmap_head);
				flush |= __kvm_zap_rmap(kvm, rmap_head, memslot);
				// dsm_debug_v("before call kvm_tdp_mmu_zap_leafs(gfn=%lld, gfn+1=%lld)\n", gfn,gfn+1);
				// flush |= kvm_tdp_mmu_zap_leafs(kvm, gfn, gfn+1, false);
				// dsm_debug_v("after call kvm_tdp_mmu_zap_leafs(gfn=%lld, gfn+1=%lld)\n", gfn,gfn+1);
				break;
			case DSM_SHARED:
				dsm_debug_v("before call kvm_tdp_mmu_write_protect_gfn(gfn=%lld)\n", gfn);
				flush |= kvm_mmu_slot_gfn_write_protect(kvm, memslot, gfn, PG_LEVEL_4K);
				dsm_debug_v("after call kvm_tdp_mmu_write_protect_gfn(gfn=%lld)\n", gfn);
				break;
			default:
				break;
		}
				  ...
				  struct kvm_page_fault *fault)
{
	...
	dsm_debug_v("kvm_tdp_mmu_page_fault: vcpu=%d fault->gfn=%lld fault.addr=%lld fault.slot=%p\n", vcpu->vcpu_id, fault->gfn, fault->addr, fault->slot);

	if (page_fault_handle_page_track(vcpu, fault))
		return RET_PF_EMULATE;

// #ifndef CONFIG_KVM_DSM
...
// #endif
...
#ifdef CONFIG_KVM_DSM
	dsm_debug_v("kvm[%d]-cpu[%d] #pf[%llx,%x](%s) gfn[%llu,%d]\n",
			vcpu->kvm->arch.dsm_id, vcpu->vcpu_id, fault->addr, fault->error_code,
			write ? "W" : (fault->error_code & PFERR_FETCH_MASK ? "RI" : "R"),
			fault->gfn, is_smm(vcpu));
	// 获取页表dsm权限
	dsm_access = kvm_dsm_vcpu_acquire_page(vcpu, &slot, fault->gfn, write);
	dsm_debug_v("dsm_access: %d, write: %d\n",dsm_access, write);
	if (dsm_access < 0) {
	...
	if (is_page_fault_stale(vcpu, fault)){
		dsm_debug_v("is_page_fault_stale: true\n");
		goto out_unlock;
	}
...
int kvm_tdp_page_fault(struct kvm_vcpu *vcpu, struct kvm_page_fault *fault)
{
	 ...
	 */
	dsm_debug_v("kvm_tdp_page_fault: vcpu=%d fault->gfn=%lld fault.addr=%lld fault.slot=%p\n", vcpu->vcpu_id, fault->gfn, fault->addr, fault->slot);
		       ...
		       void *insn, int insn_len)
{
	...
	dsm_debug_v("%s: enter kvm_mmu_page_fault(vcpu_id=%d, gpa=%llx)\n",
			       __func__, vcpu->vcpu_id, cr2_or_gpa);
	/*
	...
	if (unlikely(error_code & PFERR_RSVD_MASK)) {
		// printk(KERN_INFO "%s: call handle_mmio_page_fault(vcpu_id=%d, gpa=%llx)\n",
		// 	       __func__, vcpu->vcpu_id, cr2_or_gpa);
		r = handle_mmio_page_fault(vcpu, cr2_or_gpa, direct);
			// printk(KERN_INFO "%s: after call handle_mmio_page_fault(vcpu_id=%d, gpa=%llx)\n",
			//        __func__, vcpu->vcpu_id, cr2_or_gpa);
		if (r == RET_PF_EMULATE)
			goto emulate;
	}
	...
	if (r == RET_PF_INVALID) {
		// printk(KERN_INFO "%s: call kvm_mmu_do_page_fault(vcpu_id=%d, gpa=%llx)\n",
		// 	       __func__, vcpu->vcpu_id, cr2_or_gpa);
		r = kvm_mmu_do_page_fault(vcpu, cr2_or_gpa,
					  lower_32_bits(error_code), false,
					  &emulation_type);
		// printk(KERN_INFO "%s: after call kvm_mmu_do_page_fault(vcpu_id=%d, gpa=%llx)\n",
		// 	       __func__, vcpu->vcpu_id, cr2_or_gpa);
				   ...
				   struct kvm_memory_slot *slot)
{
	...
	kvm_dsm_remove_memslot(kvm, slot);
}
