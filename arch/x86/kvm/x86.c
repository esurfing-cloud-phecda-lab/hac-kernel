...
static void record_steal_time(struct kvm_vcpu *vcpu)
{
	...
	struct kvm_memslots *slots, *slots_acquire;
	...
	if (kvm_dsm_vcpu_acquire(vcpu, &slots_acquire, ghc->gpa,
		sizeof(struct kvm_steal_time), true) < 0)
		return;
 ...
 out_dsm:
	kvm_dsm_vcpu_release(vcpu, slots_acquire, vcpu->arch.st.cache.gpa,
...
static void kvm_steal_time_set_preempted(struct kvm_vcpu *vcpu)
{
	...
	struct kvm_steal_time __user *st;
	struct kvm_memslots *slots;
	struct kvm_memslots *slots_acquire;
	...
	if (kvm_dsm_vcpu_acquire(vcpu, &slots_acquire, ghc->gpa, sizeof(preempted), true) < 0)
		return;
	...
	mark_page_dirty_in_slot(vcpu->kvm, ghc->memslot, gpa_to_gfn(ghc->gpa));
	kvm_dsm_vcpu_release(vcpu, slots_acquire, ghc->gpa, sizeof(preempted));
}
				      ...
				      struct x86_exception *exception)
{
	...
	while (bytes) {
		...
		ret = kvm_vcpu_read_guest_page(vcpu, gfn, data,
					       offset, toread);
				...
				struct x86_exception *exception)
{
	...
	ret = kvm_vcpu_read_guest_page(vcpu, gfn, val,
				       offset, bytes);
	kvm_dsm_vcpu_release_page(vcpu, slot, gfn);
	if (unlikely(ret < 0))
		return X86EMUL_IO_NEEDED;
					  ...
					  enum kvm_mr_change change)
{
	...
	// 将新的slot绑定到kvm的dsm上
	if ((change == KVM_MR_CREATE) && (kvm_dsm_register_memslot_hva(kvm, slot, npages))) {
		kvm_page_track_free_memslot(slot);
		goto out_free;
	}
				   ...
				   enum kvm_mr_change change)
{
	...
	if (change == KVM_MR_CREATE || change == KVM_MR_MOVE) {
		...
		return kvm_alloc_memslot_metadata(kvm, new, change);
	}
...
int kvm_arch_prepare_memory_region_dsm(struct kvm *kvm,
				   const struct kvm_memory_slot *old,
				   struct kvm_memory_slot *new,
				   enum kvm_mr_change change)
{
	if (change == KVM_MR_DELETE || change == KVM_MR_FLAGS_ONLY)
		return 0;
	return kvm_dsm_add_memslot(kvm, new, new->as_id);
}
...
static inline int apf_put_user_ready(struct kvm_vcpu *vcpu, u32 token)
{
	...
	kvm_dsm_vcpu_release(vcpu, slots, vcpu->arch.apf.data.gpa, sizeof(token));				
	return ret;
}
